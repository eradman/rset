...
try 'Scan multiple files with more than one label' do
  ...
  expected = <<~OUTPUT
    ...
    \e[33m10\e[0m\t\e[4mpkg_add \e[0mrsync-- ruby%3.2
    input/t460s.pln (\e[36mdesktop\e[0m)
    \e[33m22\e[0m\t\e[4mpkg_add \e[0mhermit-font vim--gtk2
  OUTPUT
  eq out, expected
...
try 'Find more than one match per label' do
  ...
  expected = <<~OUTPUT
    input/t460s.pln (\e[36mcommon packages\e[0m)
    \e[33m11\e[0m\techo "inet 172.16.0.1/16" > \e[4m/etc/hostname\e[0m.vether0
    \e[33m12\e[0m\techo "add vether0" > \e[4m/etc/hostname\e[0m.bridge0
  OUTPUT
  eq out, expected
  eq status.success?, true
end
